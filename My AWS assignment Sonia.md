<h1 align="center">Hi 👋, I'm SONIA TIWARI</h1>
<p align="center">
...
**Answer:**  
**Steps Performed:**
1. Created a new IAM user named `sonia-pw `.
2. Created a custom IAM policy to allow limited S3 access (List, Get, Put, Delete) only for the bucket: `S3Access-sonia-pw-bucket`.
...
1. Created an S3 bucket named `sonia-pw-bucket`.
2. Enabled versioning on the bucket through the AWS Management Console.
3. Uploaded a JSON file `sonia-pw-json.json` with the original data:
...
```
4. Uploaded a second version of the same file `sonia-pw-json.json` with updated age:
...
1. Created and applied a lifecycle policy on the S3 bucket (`Sonia-pw-bucket`) to automatically transition objects to Glacier storage after 30 days and delete them after 90 day.
...
I logged into the AWS Management Console, navigated to DynamoDB, and created a new table named `SoniaTable` with `id` as the Partition Key (string).
...
1. **Created an S3 bucket** named `Sonia-Tiwari-log`.
...
**S3 Dataset Source**
- **Bucket Name**: `sonia-pw-bucket`
- **Folder**: `csv-data/`
- **CSV File Example**: `s3://sonia-pw-bucket/csv-data/pw-sample.csv`
...
```bash
s3://sonia-pw-bucket/parquet-data/part-00000-fa7eba9f-21d5-453f-a382-463dab9d8c3c-c000.snappy.parquet
...
```
**COPY Command Used:**
To load the CSV file from my S3 bucket `(s3://sonia-pw-bucket/csv-data/sales_data.csv)` into the `sales_data` table, I used the following COPY command:

```sql
COPY sales_data
FROM 's3://sonia-pw-bucket/csv-data/sales_data.csv'
...
- I uploaded my dataset (CSV files) to the S3 bucket: `s3://sonia-pw-bucket/csv-data/`.
...
I connected **Amazon QuickSight** to **Amazon Athena** as the data source. The data was stored in an **S3 bucket** (`s3://sonia-pw-us/sales/`) and queried using **Athena** after setting up the external table.
